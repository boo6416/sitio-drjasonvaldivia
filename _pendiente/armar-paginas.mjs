...
const FIGURAS = "C:/Users/boo_v/Documents/figuras-quiru";
const PLANTILLA = join(SITIO, "cirugia-de-tiroides");
...
function estatico(d, waHref) {
  const li = (xs) => xs.map((x) => `<li>${esc(x)}</li>`).join("");
  return `<div class="pre-estatico">
  <nav class="pre-migas"><a href="/">Inicio</a> / <span>${esc(d.breadcrumbLabel)}</span></nav>
  <p class="pre-kicker">${esc(d.kicker)}</p>
  <h1>${esc(d.title)}</h1>
  <p class="pre-lede">${esc(d.lede)}</p>
  <p class="pre-cta">
    <a class="pre-boton" href="/citas/">Agendar consulta</a>
    <a class="pre-boton pre-wa" href="${waHref}">WhatsApp citas</a>
  </p>
  ...
  <h2>Preguntas frecuentes</h2>${d.faqs.map((f) => `<h3>${esc(f.q)}</h3><p>${esc(f.a)}</p>`).join("")}
  <h2>${esc(d.finalTitle)}</h2><p>${esc(d.finalText)}</p>
  <p class="pre-cta"><a class="pre-boton" href="/citas/">Agendar consulta</a></p>
</div>`;
}

function objetoDatos(d, waHref) {
...
export function armar(pagina) {
  const { slug, figura, metaTitle, metaDescription, d } = pagina;
  const url = `${BASE}/${slug}/`;
  const dir = join(SITIO, slug);
  mkdirSync(dir, { recursive: true });

  d.slotId = `${slug}-hero`;
  d.imagePlaceholder = `Ilustración: ${d.breadcrumbLabel.toLowerCase()}`;
  const waHref = `https://wa.me/523313470335?text=${encodeURIComponent("Hola doctor, tengo dudas sobre " + d.breadcrumbLabel.toLowerCase())}`;
  ...
  /* Estas paginas nacen SOLO en espanol. Declarar un hreflang="en" que apunta a
     una pagina que no existe es peor que no declararlo: Google lo trata como
     error de rastreo. Cuando existan las inglesas se agrega. */
  s = s.replace(/<link rel="alternate" hreflang="es"[^>]*>\s*<link rel="alternate" hreflang="en"[^>]*>/,
  ...
  s = s.replace(ld, jsonLd(pagina, d, url));
  ...
  s = s.slice(0, iEst) + estatico(d, waHref) + s.slice(finEst + 6);
  ...
  s = s.slice(0, ini) + objetoDatos(d, waHref) + s.slice(fin);
  ...
  const fig = join("C:/Users/boo_v/.claude/jobs/1690a97e/tmp/imagenes", `${slug}.png`);
  if (!existsSync(fig)) throw new Error(`falta la figura ${fig}`);
